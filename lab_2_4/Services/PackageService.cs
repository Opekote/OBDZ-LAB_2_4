...
public class PackageService
{
    ...
    public void LoadPackagesWithEagerLoading()
    {
        ...
        var shipmentIds = packages.Select(p => p.Shipment.ShipmentID).Distinct().ToList();
        var shipments = _session.Query<Shipment>()
            .Where(s => shipmentIds.Contains(s.ShipmentID))
            .ToList();

        foreach (var package in packages)
        {
            var shipment = shipments.FirstOrDefault(s => s.ShipmentID == package.Shipment.ShipmentID);
            Console.WriteLine($"Package {package.PackageID}: {package.PackageType}: {package.ContentDescription}");
            if (shipment != null)
            {
                Console.WriteLine($" - Shipment ID: {shipment.ShipmentID}, Tracking Number: {shipment.TrackingNumber}");
            }
            else
            {
                Console.WriteLine(" - No shipment information available");
    ...
    public void LoadPackagesWithExplicitLoading()
    {
        var packages = _session.Query<Package>().ToList();

        foreach (var package in packages)
        {
            NHibernateUtil.Initialize(package.Shipment);
